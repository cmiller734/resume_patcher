...
def build_package(package_dir: Path, zip_path: Path, *, forbidden_base: bool = False) -> None:
    for name in [
        MASTER_RESUME,
        "resume_patcher.py",
        ...
        shutil.copy2(REPO_ROOT / name, package_dir / name)

    write_replacements(package_dir / "replacements.json")
    manifest = json.loads((REPO_ROOT / "resume_package_manifest.json").read_text(encoding="utf-8"))

    if forbidden_base:
        shutil.copy2(REPO_ROOT / MASTER_RESUME, package_dir / FORBIDDEN_RESUME)
        manifest["master_resume_path"] = FORBIDDEN_RESUME
        manifest["required_files"].append(FORBIDDEN_RESUME)

    (package_dir / "resume_package_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
